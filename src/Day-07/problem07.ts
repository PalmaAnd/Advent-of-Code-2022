...
const input = fs.readFileSync("src/Day-07/input.txt", "utf-8").replace(/\r/g, "");
...
commands.forEach((element) => {
  ...
  if (currentValue.match(/[$] /gm) && currentValue.includes("cd")) {
    ...
    const currentDirectory = currentValue.substring(currentValue.indexOf("cd") + 3);
    ...
    if (currentDirectory != "..") {
      ...
      // Go up one directory
      lastDir = lastDir.substring(0, lastDir.lastIndexOf("/", lastDir.length - 2) + 1);
    }
  } else if (currentValue.match(/\d /gm)) { // If the command is a "du" command
